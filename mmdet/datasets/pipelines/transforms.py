...
class Resize(object):
    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            ...
            img3, scale_factor = mmcv.imrescale(
                results['img'][2], results['scale'], return_scale=True)
            img = [img1,img2,img3]
            # img = [img1]
        else:
            img1, scale_factor = mmcv.imrescale(
        ...
        results['pad_shape'] = img[0].shape  # in case that there is no padding
    ...
    def _pad_img(self, results):
        if self.size is not None:
            ...
            padded_img = [mmcv.impad_to_multiple(
                ...
                mmcv.impad_to_multiple(
                results['img'][2], self.size_divisor, pad_val=self.pad_val)
                ]
            # padded_img = [mmcv.impad_to_multiple(
            #     results['img'][0], self.size_divisor, pad_val=self.pad_val)
            #     ]
        # print(padded_img[0].shape)
...
class Normalize(object):
    ...
    def __call__(self, results):
        ...
        # results['img'] = [mmcv.imnormalize(results['img'][0], self.mean, self.std,
        #                                   self.to_rgb)]
        results['img'] = [mmcv.imnormalize(results['img'][0], self.mean, self.std,
                                          self.to_rgb),
                        mmcv.imnormalize(results['img'][1], self.mean, self.std,
                                          self.to_rgb),
                        mmcv.imnormalize(results['img'][2], self.mean, self.std,
                                          self.to_rgb)]
        # print(results['img'][0].shape)
