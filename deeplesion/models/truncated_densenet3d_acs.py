...
def _bn_function_factory(norm, relu, conv, thickness):
    def bn_function(*inputs):
        ...
        # print("Here",concated_features.shape)
...
class _DenseLayer(nn.Sequential):
    ...
    def forward(self, *prev_features, thickness):
        ...
        # print("after1",bottleneck_output.shape)
        new_features = self.conv2(self.relu2(self.norm2(bottleneck_output)))
        if self.drop_rate > 0:
            new_features = F.dropout(new_features, p=self.drop_rate,
                                     training=self.training)
        # print("after2",new_features.shape)
        return new_features
        ...
        for key in list(state_dict.keys()):
            ...
            if state_dict[key].dim() == 4:           
                ...
                if t0 == 3:
                    ...
                    # state_dict1[new_key] = state_dict1[new_key][:,:3,...]
                
                # state_dict1[new_key] = state_dict1[new_key][:,:int(t0/3),...]
            else:
                state_dict1[new_key] = state_dict[key]
        # print("new dict")
        # for key in list(state_dict1.keys()):
        #     print(key,state_dict1[key].shape)
