...
class DenseNetCustomTrunc3dACS2(nn.Module):
    def __init__(self, 
                out_dim=256,
                n_cts=3,
                fpn_finest_layer=1,
        ...
        # First convolution
        self.conv0 = AlignShiftConv(1, num_init_features, kernel_size=7, stride=2, padding=3, bias=False, ref_thickness=ref_thickness, alignshift=True)
        ...
        # TEMP
        # # First convolution
        # self.conv10 = AlignShiftConv(1, num_init_features, kernel_size=7, stride=2, padding=3, bias=False, ref_thickness=ref_thickness, alignshift=True)
        # self.norm10 = build_norm_layer(norm_cfg, num_init_features, postfix=1)[1]
        # self.relu10 = nn.ReLU(inplace=True)
        # self.pool10 = nn.MaxPool3d(kernel_size=[1, 3, 3], stride=[1, 2, 2], padding=[0, 1, 1])
        ...
        #                         bn_size=bn_size, growth_rate=growth_rate, drop_rate=drop_rate, ref_thickness=self.ref_thickness,
        #                         n_fold=self.n_fold, memory_efficient=memory_efficient)
        #     self.add_module('denseblock1%d' % (i + 1), block)
        #     num_features = num_features + num_layers * growth_rate
        #     reductionz = _Reduction_z(num_features, self.n_cts)
        #     # normrelu = _StageNormRelu(num_features)
        #     # self.add_module('normrelu%d' % (i + 1), normrelu)
        #     self.add_module('reductionz1%d' % (i + 1), reductionz)
        #     if i != len(block_config) - 1:
        #         trans = _Transition(num_input_features=num_features, num_output_features=num_features // 2)
        #         self.add_module('transition1%d' % (i + 1), trans)
        #         num_features = num_features // 2
    ...
    def forward_once(self,x,thickness):

        # print("in forward once",x.shape)
        x = self.conv0(x,thickness)
        x = self.norm0(x)
        x = self.relu0(x)
        x = self.pool0(x)
        ...
        print("shape of ftmaps",ftmaps[-1].shape)
        ...
        # x1 = self.conv10(x1,thickness)
        # x1 = self.norm10(x1)
        # x1 = self.relu10(x1)
        # x1 = self.pool10(x1)

        # x1 = self.denseblock11(x1, thickness)
        # # x = self.normrelu1(x)
        # redc11 = self.reductionz11(x1)
        # x1 = self.transition11(x1)


        # x1 = self.denseblock12(x1, thickness)
        # # x = self.normrelu2(x)
        # redc12 = self.reductionz12(x1)
        # x1 = self.transition12(x1)


        # x1 = self.denseblock13(x1, thickness)
        # # x = self.normrelu3(x)
        # redc13 = self.reductionz13(x1)

        # # if self.feature_upsample:
        # ftmaps1 = [None, redc11.squeeze(2), redc12.squeeze(2), redc13.squeeze(2)]
        # x1 = self.lateral14(ftmaps1[-1])
        # for p in range(3, self.fpn_finest_layer - 1, -1):
        #     x1 = F.interpolate(x1, scale_factor=2, mode="nearest")
        #     y1 = ftmaps1[p-1]
        #     lateral1 = getattr(self, 'lateral1%d' % p)(y1)
        #     x1 += lateral1
    ...
    def forward(self, x, thickness):
        ...
        # print("in backbone",len(x))
        x = self.forward_once(x,thickness)
        # x = torch.cat([x,x1],dim=1)
        # x1 = self.forward_once(x1,thickness)
