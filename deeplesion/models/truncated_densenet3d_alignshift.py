...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, n_fold, memory_efficient=False, ref_thickness=None):
        ...
        self.add_module('conv1', AlignShiftConv(num_input_features, bn_size *
                                           ...
                                           bias=False, ref_thickness=ref_thickness, n_fold=n_fold,groups=3)),
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        self.add_module('conv', AlignShiftConv(num_input_features, num_output_features,
                                          kernel_size=1, stride=1, bias=False, alignshift=False,groups=3))
...
class DenseNetCustomTrunc3dAlign(nn.Module):
    def __init__(self, 
        ...
        self.n_fold = n_fold
        assert self.depth in [161]
        if self.depth == 161:
            num_init_features = 96
            growth_rate = 48
            block_config = (6, 12, 36)
            # self.in_dim = [64, 256, 512, 1024]
            self.in_dim = [96, 384, 768, 2112]
        bn_size = 4
        drop_rate = 0

        # First convolution
        self.conv0 = AlignShiftConv(3, num_init_features, kernel_size=7, stride=2, padding=3, bias=False, alignshift=False,groups=3)
    ...
    def init_weights(self, pretrained=True):
        ...
        for key in list(state_dict.keys()):
            ...
            if state_dict[key].dim() == 4:           
                ...
                # print(state_dict1[new_key].shape)
                d0 =state_dict1[new_key].shape[0]
                # if 'transition' in new_key:
                #     print(new_key,d0)
                    # state_dict1[new_key] = state_dict1[new_key][:int(d0/3)]
